...
/// continues until the next `### `, `## `, a `<!-- headroom:` marker, or end
/// of file. Returns `(cleaned_text, lines_removed)`.
///
/// The marker stop is what keeps the learn block's `<!-- headroom:learn:end -->`
/// alive: when error recovery is the block's LAST subsection, the marker sits
/// between it and the next heading, and this loop ate it (headroom-desktop and
/// headroom-web MEMORY.md, 2026-09-02 and again 2026-09-06). Without the end
/// marker the wheel's writer replaces nothing and the desktop parser sees no
/// block, so learnings silently stop landing.
...
mod tests {
    ...
    /// The 2026-09-02 / 2026-09-06 loss: error recovery as the LAST subsection
    /// of the learn block, so the end marker is the next non-bullet line.
    #[test]
    fn strip_stops_at_the_learn_end_marker() {
        let input = "<!-- headroom:learn:start -->\n\
            ## Headroom Learned Patterns\n\
            *Auto-generated by `headroom learn` on 2026-09-06*\n\n\
            ### Learned: error recovery\n\
            - bogus rule\n\
            <!-- headroom:learn:end -->\n\n\
            ## Manual memory index\n\
        ...
        let (cleaned, removed) = strip_error_recovery_subsection(input);
        assert_eq!(removed, 2);
        assert!(cleaned.contains("<!-- headroom:learn:end -->"), "{cleaned}");
        assert!(!cleaned.contains("bogus rule"));
        assert!(cleaned.contains("## Manual memory index\n- keep\n"));
        // Idempotent on the cleaned result.
        assert_eq!(strip_error_recovery_subsection(&cleaned).1, 0);
    }
